...
var booksCounter = {
    //posição da lista de livros
    x:0.3, 
    y:8,
    num:5 //numero maximo de livros
};
 ...
 */
Maze.Level7.currentStudent = null;
// Maze.Level7.taskActivate = false;
/**
 * Background and other elements
 */
Maze.Level7.VIEW = {
    background: 'maze/fundobiblioteca.jpg',
 ...
 */
Maze.Level7.DrawMap = function(svg){

    //Create cenario:
    if (Maze.Level7.VIEW.background) {
        var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
        tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level7.VIEW.background);
    ...
    if (Maze.Level7.VIEW.graph) {
        ...
        for (var k = 0; k < Maze.ROWS; k++) {
          ...
          h_line.setAttribute('stroke', Maze.Level7.VIEW.graph);
        ...
        for (var k = 0; k < Maze.COLS; k++) {
          ...
          v_line.setAttribute('stroke', Maze.Level7.VIEW.graph);
    ...
    for (var y = 0; y < Maze.ROWS; y++) {
        for (var x = 0; x < Maze.COLS; x++) {
            ...
            // Tile sprite.
            var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
            tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level7.VIEW.tiles);
    ...
    for (var y = 0; y < Maze.ROWS; y++) {
        for (var x = 0; x < Maze.COLS; x++) {
            if (Maze.map[y][x] == Maze.SquareType.START) {
                ...
                var std = new Maze.Avatar(cont_start, Maze.startDirection, x, y, Maze.Level7.VIEW.skin);
                student.push(std);
                cont_start++;
    ...
    var cont_book = 0;
    for(var i=0.5; i<10.5; i++){
        bookslist_[cont_book++] = {x: 0.3, y: i/1.5};
    }

    //Set the current student as the first student
    Maze.Level7.currentStudent = student[0];
...
Maze.Level7.AddBooks = function(){
    ...
    for(var i=0; i < booksCounter.num; i++){
        books[i] = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
        books[i].id = book.concat(i.toString());
        books[i].setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level7.VIEW.book);
        books[i].setAttribute('height', 45);
        books[i].setAttribute('width', 35);
    ...
    for(var i=0; i<=booksCounter.num; i++){
        ...
        bookscount[i].setAttribute('x', Maze.SQUARE_SIZE * (booksCounter.x + 0.5) -
        bookscount[i].getAttribute('width') / 2);
        bookscount[i].setAttribute('y', Maze.SQUARE_SIZE * (booksCounter.y + 0.6) -
        bookscount[i].getAttribute('height'));
 ...
 */
Maze.Level7.AddSprites = function(svg, document){

    // Create finish markers.
    var finishMarkers = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
    finishMarkers.id = 'finish';
    finishMarkers.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level7.VIEW.finishMarker);
    ...
    pegmanIcon.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level7.VIEW.skin);
    ...
    estudante2.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level7.VIEW.skin);
 ...
 */
Maze.Level7.Reset = function(first){
    ...
    if (first) {
        ...
        pidList.push(setTimeout(function() {
            ...
            Maze.Level7.Schedule([student[0].startLoc.x, student[0].startLoc.y, student[0].startDirection * 4],
        ...
        pidList.push(setTimeout(function() {
            Maze.stepSpeed = speed;
            Maze.Level7.Schedule([student[1].startLoc.x, student[1].startLoc.y, student[1].startDirection * 4],
                        [student[1].startLoc.x, student[1].startLoc.y, student[1].startDirection * 4 - 4], student[1].id);
                        student[1].startDirection++;
        }, Maze.stepSpeed * 5));

    } else {
        Maze.Level7.DisplayStudent(0, student[0].startLoc.x, student[0].startLoc.y, Maze.startDirection * 4);
        Maze.Level7.DisplayStudent(1, student[1].startLoc.x, student[1].startLoc.y, Maze.startDirection * 4);
    }
...
Maze.Level7.ExecuteFirst = function(){
    ...
    var interpreter = new Interpreter(code, Maze.Level7.InitInterpreter);
    ...
    try {
        ...
        Maze.result = Maze.Level7.NotDone() ?
    ...
    Maze.Level7.Reset(false);
    Maze.Level7.PreAnimate(0);
    pidList.push(setTimeout(Maze.Level7.Animate, 150));
};

Maze.Level7.PreAnimate = function(id){
    ...
    Maze.Level7.PreAnimate(id);
};


Maze.Level7.Execute = function(){
    ...
    Maze.result = Maze.Level7.NotDone() ? Maze.ResultType.FAILURE : Maze.ResultType.SUCCESS;
    ...
    if (Maze.result == Maze.ResultType.SUCCESS) {
        ...
        executionTime = document.getElementById("number").innerHTML;

        var doneDiv = document.getElementById('dialogLinesText');
        var exectime = document.createElement('P');
        exectime.innerText = executionTime.toString().concat(' seconds!');
        // doneDiv.insertAdjacentElement(doneDiv, exectime);        
...
Maze.Level7.Animate = function(){

    var action1 = action_std1[++std1];
    var action2 = action_std2[++std2];

    if(action1)
        Maze.Level7.AnimateMove(student[0], action1);
    
    if(action2)
        Maze.Level7.AnimateMove(student[1], action2);
    
    pidList.push(setTimeout(Maze.Level7.Animate, Maze.stepSpeed * 4));
}

Maze.Level7.AnimateMove = function(avatar, action){

    BlocklyInterface.highlight(action[1]);

    switch (action[0]) {
        case 'north':
        Maze.Level7.Schedule([avatar.startLoc.x, avatar.startLoc.y, avatar.startDirection * 4],
        ...
        case 'look_north':
        Maze.Level7.ScheduleLook(Maze.DirectionType.NORTH, avatar.startLoc.x, avatar.startLoc.y);
        break;

        case 'look_east':
        Maze.Level7.ScheduleLook(Maze.DirectionType.EAST, avatar.startLoc.x, avatar.startLoc.y);
        break;
        
        case 'look_south':
        Maze.Level7.ScheduleLook(Maze.DirectionType.SOUTH, avatar.startLoc.x, avatar.startLoc.y);
        break;
        
        case 'look_west':
        Maze.Level7.ScheduleLook(Maze.DirectionType.WEST, avatar.startLoc.x, avatar.startLoc.y);
        break;
        
        case 'fail_forward':
        Maze.Level7.ScheduleFail(true, avatar);
        break;
        
        case 'fail_backward':
        Maze.Level7.ScheduleFail(false, avatar);
        break;
 ...
 */
Maze.Level7.InitInterpreter = function(interpreter, scope){

    // API
    var wrapper;

    wrapper = function(id) {
        Maze.Level7.move(0, id, Maze.Level7.currentStudent);
    };
    interpreter.setProperty(scope, 'moveForward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Maze.Level7.move(2, id, Maze.Level7.currentStudent);
    };
    interpreter.setProperty(scope, 'moveBackward',
        interpreter.createNativeFunction(wrapper));
        
    wrapper = function(id) {
        Maze.Level7.turn(0, id, Maze.Level7.currentStudent);
    };
    interpreter.setProperty(scope, 'turnLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Maze.Level7.turn(1, id, Maze.Level7.currentStudent);
    };
    interpreter.setProperty(scope, 'turnRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
    return Maze.Level7.isPath(0, id, Maze.Level7.currentStudent);
    };
    ...
    //Students control:
    wrapper = function(id) {
        return Maze.Level7.isStudent(0, id);
    };
    interpreter.setProperty(scope, 'isStudent0',
    interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Maze.Level7.isStudent(1, id);
    };
    interpreter.setProperty(scope, 'isStudent1',
    interpreter.createNativeFunction(wrapper));
   
    wrapper = function(id) {
        return Maze.Level7.isPath(1, id, Maze.Level7.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Maze.Level7.isPath(2, id, Maze.Level7.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathBackward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Maze.Level7.isPath(3, id, Maze.Level7.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function() {
        return Maze.Level7.NotDone();
    };
    ...
    //Books == 0
    wrapper = function() {
        return Maze.Level7.ReturnBook(Maze.Level7.currentStudent.startLoc.x, Maze.Level7.currentStudent.startLoc.y);
    };
 ...
 */
Maze.Level7.move = function(direction, id, avatar) {

    if (!Maze.Level7.isPath(direction, null, avatar)) {
 ...
 * @param {string} id ID of block that triggered this action.
 */
Maze.Level7.turn = function(direction, id, avatar) {
 ...
 */
Maze.Level7.isPath = function(direction, id, avatar) {
 ...
 */
Maze.Level7.isStudent = function(student_id, id) {
    var command;
    ...
    switch(student_id) {
      ...
      case 1:
        command = 'student1';
        Maze.Level7.currentStudent = student[1];
        break;
      default:
        break;
    }
 ...
 */
Maze.Level7.Schedule = function(startPos, endPos, est) {

    var deltas = [(endPos[0] - startPos[0])/4, (endPos[1] - startPos[1])/4, (endPos[2] - startPos[2])/4];

    pidList.push(setTimeout(function() {
    Maze.Level7.DisplayStudent(est, startPos[0] + deltas[0],
        startPos[1] + deltas[1],
        Maze.constrainDirection16(startPos[2] + deltas[2]));
    }, Maze.stepSpeed));

    pidList.push(setTimeout(function() {
        Maze.Level7.DisplayStudent(est, startPos[0] + deltas[0] * 2,
            startPos[1] + deltas[1] * 2,
            Maze.constrainDirection16(startPos[2] + deltas[2] * 2));
    }, Maze.stepSpeed));

    pidList.push(setTimeout(function() {
    Maze.Level7.DisplayStudent(est, startPos[0] + deltas[0] * 3,
        startPos[1] + deltas[1] * 3,
        Maze.constrainDirection16(startPos[2] + deltas[2] * 3));
    }, Maze.stepSpeed * 2));

    pidList.push(setTimeout(function() {
        Maze.Level7.DisplayStudent(est, endPos[0], endPos[1],
 ...
 */
Maze.Level7.ScheduleFail = function(forward, avatar) {
    ...
    Maze.Level7.DisplayStudent(avatar.id, avatar.startLoc.x + deltaX,
                        avatar.startLoc.y + deltaY,
                        direction16);

    BlocklyGames.workspace.getAudioManager().play('fail', 0.5);
    pidList.push(setTimeout(function() {
        Maze.Level7.DisplayStudent(avatar.id, avatar.startLoc.x,
                            avatar.startLoc.y,
                            direction16);
    }, Maze.stepSpeed));
    ...
    pidList.push(setTimeout(function() {
        Maze.Level7.DisplayStudent(avatar.id, avatar.startLoc.x, avatar.startLoc.y, direction16);
    }, Maze.stepSpeed * 3));
};

Maze.Level7.ScheduleFinish = function(sound) {
    var direction16 = Maze.constrainDirection16(Maze.Level7.currentStudent.direction * 4);
    Maze.Level7.DisplayStudent(Maze.Level7.currentStudent.startLoc.x, Maze.Level7.currentStudent.startLoc.y, 16);
    if (sound) {
      BlocklyGames.workspace.getAudioManager().play('win', 0.5);
    }
    pidList.push(setTimeout(function() {
        Maze.Level7.DisplayStudent(Maze.Level7.currentStudent.startLoc.x, Maze.Level7.currentStudent.startLoc.y, 18);
      }, Maze.stepSpeed));
    pidList.push(setTimeout(function() {
        Maze.Level7.DisplayStudent(Maze.Level7.currentStudent.startLoc.x, Maze.Level7.currentStudent.startLoc.y, 16);
      }, Maze.stepSpeed * 2));
    pidList.push(setTimeout(function() {
        Maze.Level7.DisplayStudent(Maze.Level7.currentStudent.startLoc.x, Maze.Level7.currentStudent.startLoc.y, direction16);
      }, Maze.stepSpeed * 3));
  };
 ...
 */
Maze.Level7.DisplayStudent = function(est, x, y, d, opt_angle) {
    ...
    if(Maze.Level7.ReturnBook(x, y)){
        //Remove one book from the list and decrement the counter
        booksCounter.num--;
        //Move the student to the initial position and run the code again
        var svg = (document.getElementById('svgMaze'));
        Maze.Level7.ResetOneStd(est);
        Maze.Level7.RemoveBooks(svg, document);
        Maze.Level7.Execute();
    }
};


Maze.Level7.ResetOneStd = function(est){
    ...
    Maze.Level7.DisplayStudent(est, student[est].startLoc.x, student[est].startLoc.y, Maze.startDirection * 4);
 ...
 */
Maze.Level7.ScheduleLook = function(d, x, y) {
 ...
 */
Maze.Level7.ReturnBook = function(x, y){
 ...
 */
Maze.Level7.RemoveBooks = function(svg, document){
    var book = 'book';
    const bookremove = document.getElementById(book.concat(booksCounter.num.toString()));
    svg.removeChild(bookremove);

    var bookcont = 'booksCounter';
    var id = booksCounter.num + 1;
    const bookcontremove = document.getElementById(bookcont.concat(id.toString()));
    svg.removeChild(bookcontremove);
}

Maze.Level7.NotDone = function(){
    return booksCounter.num != 0;
};