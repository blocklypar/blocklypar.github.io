 ...
 */
Maze.Level6.currentStudent = null;
// Maze.Level6.taskActivate = false;
/**
 ...
 */
Maze.Level6.DrawMap = function(svg){

    //Create cenario:
    if (Maze.Level6.VIEW.background) {
        var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
        tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level6.VIEW.background);
    ...
    if (Maze.Level6.VIEW.graph) {
        ...
        for (var k = 0; k < Maze.ROWS; k++) {
          ...
          h_line.setAttribute('stroke', Maze.Level6.VIEW.graph);
        ...
        for (var k = 0; k < Maze.COLS; k++) {
          ...
          v_line.setAttribute('stroke', Maze.Level6.VIEW.graph);
    ...
    for (var y = 0; y < Maze.ROWS; y++) {
        for (var x = 0; x < Maze.COLS; x++) {
            ...
            // Tile sprite.
            var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
            tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level6.VIEW.tiles);
    ...
    for (var y = 0; y < Maze.ROWS; y++) {
        for (var x = 0; x < Maze.COLS; x++) {
            if (Maze.map[y][x] == Maze.SquareType.START) {
                ...
                var std = new Maze.Avatar(cont_start, Maze.startDirection, x, y, Maze.Level6.VIEW.skin);
                student.push(std);
                cont_start++;
...
Maze.Level6.AddBooks = function(){
    ...
    for(var i=0; i < booksCounterNum; i++){
        ...
        books[i].setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level6.VIEW.book);
 ...
 */
Maze.Level6.AddSprites = function(svg, document){

    // Create finish markers.
    var finishMarkers = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
    finishMarkers.id = 'finish';
    finishMarkers.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level6.VIEW.finishMarker);
    ...
    pegmanIcon.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level6.VIEW.skin);
    ...
    estudante2.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Maze.Level6.VIEW.skin);
 ...
 */
Maze.Level6.Reset = function(first){
    ...
    if (first) {
        ...
        pidList.push(setTimeout(function() {
            Maze.Level6.Schedule([student[0].startLoc.x, student[0].startLoc.y, student[0].startDirection * 4],
        ...
        pidList.push(setTimeout(function() {
            Maze.Level6.Schedule([student[1].startLoc.x, student[1].startLoc.y, student[1].startDirection * 4],
...
Maze.Level6.ExecuteFirst = function(){
    ...
    var interpreter = new Interpreter(code, Maze.Level6.InitInterpreter);
    ...
    try {
        ...
        Maze.result = Maze.Level6.NotDone() ?
    ...
    Maze.Level6.Reset(false);
    Maze.Level6.PreAnimate(0);
...
Maze.Level6.Execute = function(){
    ...
    Maze.stepSpeed = speed;
    Maze.result = Maze.Level6.NotDone() ? Maze.ResultType.FAILURE : Maze.ResultType.SUCCESS;
...
Maze.Level6.AnimateMove = function(avatar, action){

    BlocklyInterface.highlight(action[1]);

    switch (action[0]) {
        case 'north': //value: 1
        Maze.Level6.Schedule([avatar.startLoc.x, avatar.startLoc.y, avatar.startDirection * 4],
        ...
        Maze.Level6.ScheduleLook(Maze.DirectionType.NORTH, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;

        case 'look_east': //value 0.5
        Maze.Level6.ScheduleLook(Maze.DirectionType.EAST, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;
        
        case 'look_south': //value 0.5
        Maze.Level6.ScheduleLook(Maze.DirectionType.SOUTH, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;
        
        case 'look_west': //value 0.5
        Maze.Level6.ScheduleLook(Maze.DirectionType.WEST, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;
        
        case 'fail_forward':
        Maze.Level6.ScheduleFail(true, avatar);
        break;
        
        case 'fail_backward':
        Maze.Level6.ScheduleFail(false, avatar);
        break;
 ...
 */
Maze.Level6.InitInterpreter = function(interpreter, scope){

    // API
    var wrapper;

    wrapper = function(id) {
        Maze.Level6.move(0, id, Maze.Level6.currentStudent);
    };
    interpreter.setProperty(scope, 'moveForward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Maze.Level6.move(2, id, Maze.Level6.currentStudent);
    };
    interpreter.setProperty(scope, 'moveBackward',
        interpreter.createNativeFunction(wrapper));
        
    wrapper = function(id) {
        Maze.Level6.turn(0, id, Maze.Level6.currentStudent);
    };
    interpreter.setProperty(scope, 'turnLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Maze.Level6.turn(1, id, Maze.Level6.currentStudent);
    };
    interpreter.setProperty(scope, 'turnRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
    return Maze.Level6.isPath(0, id, Maze.Level6.currentStudent);
    };
    ...
    //Students control:
    wrapper = function(id) {
        return Maze.Level6.isStudent(0, id);
    };
    ...
    wrapper = function(id) {
        return Maze.Level6.isPath(1, id, Maze.Level6.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Maze.Level6.isPath(2, id, Maze.Level6.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathBackward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Maze.Level6.isPath(3, id, Maze.Level6.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function() {
        return Maze.Level6.NotDone();
    };
    ...
    //Books == 0
    wrapper = function() {
        return Maze.Level6.ReturnBook(Maze.Level6.currentStudent.startLoc.x, Maze.Level6.currentStudent.startLoc.y);
    };
 ...
 */
Maze.Level6.move = function(direction, id, avatar) {

    if (!Maze.Level6.isPath(direction, null, avatar)) {
 ...
 * @param {string} id ID of block that triggered this action.
 */
Maze.Level6.turn = function(direction, id, avatar) {
 ...
 */
Maze.Level6.isPath = function(direction, id, avatar) {
 ...
 */
Maze.Level6.isStudent = function(student_id, id) {
    var command;
 ...
 */
Maze.Level6.Schedule = function(startPos, endPos, est) {

    var deltas = [(endPos[0] - startPos[0])/4, (endPos[1] - startPos[1])/4, (endPos[2] - startPos[2])/4];

    pidList.push(setTimeout(function() {
    Maze.Level6.DisplayStudent(est, startPos[0] + deltas[0],
        startPos[1] + deltas[1],
        Maze.constrainDirection16(startPos[2] + deltas[2]));
    }, Maze.stepSpeed));

    pidList.push(setTimeout(function() {
        Maze.Level6.DisplayStudent(est, startPos[0] + deltas[0] * 2,
            startPos[1] + deltas[1] * 2,
            Maze.constrainDirection16(startPos[2] + deltas[2] * 2));
    }, Maze.stepSpeed));

    pidList.push(setTimeout(function() {
    Maze.Level6.DisplayStudent(est, startPos[0] + deltas[0] * 3,
        startPos[1] + deltas[1] * 3,
        Maze.constrainDirection16(startPos[2] + deltas[2] * 3));
    }, Maze.stepSpeed * 2));

    pidList.push(setTimeout(function() {
        Maze.Level6.DisplayStudent(est, endPos[0], endPos[1],
 ...
 */
Maze.Level6.ScheduleFail = function(forward, avatar) {
    ...
    Maze.Level6.DisplayStudent(avatar.id, avatar.startLoc.x + deltaX,
                        avatar.startLoc.y + deltaY,
                        direction16);

    BlocklyGames.workspace.getAudioManager().play('fail', 0.5);
    pidList.push(setTimeout(function() {
        Maze.Level6.DisplayStudent(avatar.id, avatar.startLoc.x,
                            avatar.startLoc.y,
                            direction16);
    }, Maze.stepSpeed));
    ...
    pidList.push(setTimeout(function() {
        Maze.Level6.DisplayStudent(avatar.id, avatar.startLoc.x, avatar.startLoc.y, direction16);
    }, Maze.stepSpeed * 3));
};

Maze.Level6.ScheduleFinish = function(sound) {
    var direction16 = Maze.constrainDirection16(Maze.Level6.currentStudent.direction * 4);
    Maze.Level6.DisplayStudent(Maze.Level6.currentStudent.startLoc.x, Maze.Level6.currentStudent.startLoc.y, 16);
    if (sound) {
    ...
    pidList.push(setTimeout(function() {
        Maze.Level6.DisplayStudent(Maze.Level6.currentStudent.startLoc.x, Maze.Level6.currentStudent.startLoc.y, direction16);
      }, Maze.stepSpeed * 3));
  };
...
Maze.Level6.ResetOneStd = function(est){
 ...
 */
Maze.Level6.ScheduleLook = function(d, x, y) {
 ...
 */
Maze.Level6.ReturnBook = function(x, y){
 ...
 */
Maze.Level6.RemoveBooks = function(svg, document){
    var book = 'book';
