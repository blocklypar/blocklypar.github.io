 ...
 */
Parallel.Level3.currentStudent = null;
// Parallel.Level3.taskActivate = false;
/**
 * Background and other elements
 */
Parallel.Level3.VIEW = {
 ...
 */
Parallel.Level3.DrawMap = function(svg){

    //Create cenario:
    if (Parallel.Level3.VIEW.background) {
        var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
        tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Parallel.Level3.VIEW.background);
    ...
    if (Parallel.Level3.VIEW.graph) {
        ...
        for (var k = 0; k < Parallel.ROWS; k++) {
          ...
          h_line.setAttribute('stroke', Parallel.Level3.VIEW.graph);
        ...
        for (var k = 0; k < Parallel.COLS; k++) {
          ...
          v_line.setAttribute('stroke', Parallel.Level3.VIEW.graph);
    ...
    for (var y = 0; y < Parallel.ROWS; y++) {
        for (var x = 0; x < Parallel.COLS; x++) {
            ...
            // Tile sprite.
            var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
            tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Parallel.Level3.VIEW.tiles);
    ...
    for (var y = 0; y < Parallel.ROWS; y++) {
        for (var x = 0; x < Parallel.COLS; x++) {
            if (Parallel.map[y][x] == Parallel.SquareType.START) {
                ...
                var std = new Parallel.Avatar(cont_start, Parallel.startDirection, x, y, Parallel.Level3.VIEW.skin);
                student.push(std);
                cont_start++;
    ...
    Parallel.Level3.currentStudent = student[0];
 ...
 */
Parallel.Level3.AddBooks = function(){
    ...
    for(var i=0; i < booksCounterNum; i++){
        ...
        books[i].setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Parallel.Level3.VIEW.book);
 ...
 */
Parallel.Level3.AddSprites = function(svg, document){

    // Create finish markers.
    var finishMarkers = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
    finishMarkers.id = 'finish';
    finishMarkers.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Parallel.Level3.VIEW.finishMarker);
    ...
    pegmanIcon.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Parallel.Level3.VIEW.skin);
    ...
    estudante2.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Parallel.Level3.VIEW.skin);
 ...
 */
Parallel.Level3.Reset = function(first){
    ...
    if (first) {
        ...
        pidList.push(setTimeout(function() {
            Parallel.Level3.Schedule([student[0].startLoc.x, student[0].startLoc.y, student[0].startDirection * 4],
        ...
        pidList.push(setTimeout(function() {
            Parallel.Level3.Schedule([student[1].startLoc.x, student[1].startLoc.y, student[1].startDirection * 4],
                        [student[1].startLoc.x, student[1].startLoc.y, student[1].startDirection * 4 - 4], student[1].id);
                        student[1].startDirection++;
        }, Parallel.stepSpeed * 4));

    } else {
        Parallel.Level3.DisplayStudent(0, student[0].startLoc.x, student[0].startLoc.y, Parallel.startDirection * 4);
        Parallel.Level3.DisplayStudent(1, student[1].startLoc.x, student[1].startLoc.y, Parallel.startDirection * 4);
    }
...
Parallel.Level3.ExecuteFirst = function(){
    ...
    var interpreter = new Interpreter(code, Parallel.Level3.InitInterpreter);
    ...
    try {
        ...
        Parallel.result = Parallel.Level3.NotDone() ?
    ...
    Parallel.Level3.Reset(false);
    Parallel.Level3.PreAnimate(0);
    pidList.push(setTimeout(Parallel.Level3.Animate, 150));
};

Parallel.Level3.PreAnimate = function(id){
    ...
    Parallel.Level3.PreAnimate(id);
};


Parallel.Level3.Execute = function(){
    ...
    Parallel.result = Parallel.Level3.NotDone() ? Parallel.ResultType.FAILURE : Parallel.ResultType.SUCCESS;
...
Parallel.Level3.Animate = function(){

    var action1 = action_std1[++std1];
    var action2 = action_std2[++std2];

    if(action1)
        Parallel.Level3.AnimateMove(student[0], action1);

    if(action2)
        Parallel.Level3.AnimateMove(student[1], action2);

    if(action1 && action2)
        Parallel.updateTime(((student[0].time + student[1].time)/2).toFixed(2));
    else if(!action1)
        Parallel.updateTime(student[1].time);
    else if(!action2)
        Parallel.updateTime(student[0].time);


    pidList.push(setTimeout(Parallel.Level3.Animate, Parallel.stepSpeed * 4));
}

Parallel.Level3.AnimateMove = function(avatar, action){

    BlocklyInterface.highlight(action[1]);

    switch (action[0]) {
        case 'north': //value: 1
        Parallel.Level3.Schedule([avatar.startLoc.x, avatar.startLoc.y, avatar.startDirection * 4],
        ...
        Parallel.Level3.ScheduleLook(Parallel.DirectionType.NORTH, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;

        case 'look_east': //value 0.5
        Parallel.Level3.ScheduleLook(Parallel.DirectionType.EAST, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;
    
        case 'look_south': //value 0.5
        Parallel.Level3.ScheduleLook(Parallel.DirectionType.SOUTH, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;
    
        case 'look_west': //value 0.5
        Parallel.Level3.ScheduleLook(Parallel.DirectionType.WEST, avatar.startLoc.x, avatar.startLoc.y);
        avatar.time = avatar.time + 0.5;
        break;
    
        case 'fail_forward':
        Parallel.Level3.ScheduleFail(true, avatar);
        break;
    
        case 'fail_backward':
        Parallel.Level3.ScheduleFail(false, avatar);
        break;
 ...
 */
Parallel.Level3.InitInterpreter = function(interpreter, scope){

    // API
    var wrapper;

    wrapper = function(id) {
        Parallel.Level3.move(0, id, Parallel.Level3.currentStudent);
    };
    interpreter.setProperty(scope, 'moveForward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Parallel.Level3.move(2, id, Parallel.Level3.currentStudent);
    };
    interpreter.setProperty(scope, 'moveBackward',
        interpreter.createNativeFunction(wrapper));
    
    wrapper = function(id) {
        Parallel.Level3.turn(0, id, Parallel.Level3.currentStudent);
    };
    interpreter.setProperty(scope, 'turnLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Parallel.Level3.turn(1, id, Parallel.Level3.currentStudent);
    };
    interpreter.setProperty(scope, 'turnRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
    return Parallel.Level3.isPath(0, id, Parallel.Level3.currentStudent);
    };
    ...
    //Students control:
    wrapper = function(id) {
        return Parallel.Level3.isStudent(0, id);
    };
    interpreter.setProperty(scope, 'isStudent0',
    interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Parallel.Level3.isStudent(1, id);
    };
    interpreter.setProperty(scope, 'isStudent1',
    interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Parallel.Level3.isPath(1, id, Parallel.Level3.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Parallel.Level3.isPath(2, id, Parallel.Level3.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathBackward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Parallel.Level3.isPath(3, id, Parallel.Level3.currentStudent);
    };
    interpreter.setProperty(scope, 'isPathLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function() {
        return Parallel.Level3.NotDone();
    };
    ...
    //Books == 0
    wrapper = function() {
        return Parallel.Level3.ReturnBook(Parallel.Level3.currentStudent.startLoc.x, Parallel.Level3.currentStudent.startLoc.y);
    };
 ...
 */
Parallel.Level3.move = function(direction, id, avatar) {

    if (!Parallel.Level3.isPath(direction, null, avatar)) {
 ...
 * @param {string} id ID of block that triggered this action.
 */
Parallel.Level3.turn = function(direction, id, avatar) {
 ...
 */
Parallel.Level3.isPath = function(direction, id, avatar) {
 ...
 */
Parallel.Level3.isStudent = function(student_id, id) {
    var command;
    ...
    switch(student_id) {
      ...
      case 1:
        command = 'student1';
        Parallel.Level3.currentStudent = student[1];
        break;
      default:
        break;
    }
 ...
 */
Parallel.Level3.Schedule = function(startPos, endPos, est) {

    var deltas = [(endPos[0] - startPos[0])/4, (endPos[1] - startPos[1])/4, (endPos[2] - startPos[2])/4];

    pidList.push(setTimeout(function() {
    Parallel.Level3.DisplayStudent(est, startPos[0] + deltas[0],
        startPos[1] + deltas[1],
        Parallel.constrainDirection16(startPos[2] + deltas[2]));
    }, Parallel.stepSpeed));

    pidList.push(setTimeout(function() {
        Parallel.Level3.DisplayStudent(est, startPos[0] + deltas[0] * 2,
            startPos[1] + deltas[1] * 2,
            Parallel.constrainDirection16(startPos[2] + deltas[2] * 2));
    }, Parallel.stepSpeed));

    pidList.push(setTimeout(function() {
    Parallel.Level3.DisplayStudent(est, startPos[0] + deltas[0] * 3,
        startPos[1] + deltas[1] * 3,
        Parallel.constrainDirection16(startPos[2] + deltas[2] * 3));
    }, Parallel.stepSpeed * 2));

    pidList.push(setTimeout(function() {
        Parallel.Level3.DisplayStudent(est, endPos[0], endPos[1],
 ...
 */
Parallel.Level3.ScheduleFail = function(forward, avatar) {
    ...
    Parallel.Level3.DisplayStudent(avatar.id, avatar.startLoc.x + deltaX,
                        avatar.startLoc.y + deltaY,
                        direction16);

    BlocklyGames.workspace.getAudioManager().play('fail', 0.5);
    pidList.push(setTimeout(function() {
        Parallel.Level3.DisplayStudent(avatar.id, avatar.startLoc.x,
                            avatar.startLoc.y,
                            direction16);
    }, Parallel.stepSpeed));
    ...
    pidList.push(setTimeout(function() {
        Parallel.Level3.DisplayStudent(avatar.id, avatar.startLoc.x, avatar.startLoc.y, direction16);
    }, Parallel.stepSpeed * 3));
};

Parallel.Level3.ScheduleFinish = function(sound) {
    var direction16 = Parallel.constrainDirection16(Parallel.Level3.currentStudent.direction * 4);
    Parallel.Level3.DisplayStudent(Parallel.Level3.currentStudent.startLoc.x, Parallel.Level3.currentStudent.startLoc.y, 16);
    if (sound) {
      BlocklyGames.workspace.getAudioManager().play('win', 0.5);
    }
    pidList.push(setTimeout(function() {
        Parallel.Level3.DisplayStudent(Parallel.Level3.currentStudent.startLoc.x, Parallel.Level3.currentStudent.startLoc.y, 18);
      }, Parallel.stepSpeed));
    pidList.push(setTimeout(function() {
        Parallel.Level3.DisplayStudent(Parallel.Level3.currentStudent.startLoc.x, Parallel.Level3.currentStudent.startLoc.y, 16);
      }, Parallel.stepSpeed * 2));
    pidList.push(setTimeout(function() {
        Parallel.Level3.DisplayStudent(Parallel.Level3.currentStudent.startLoc.x, Parallel.Level3.currentStudent.startLoc.y, direction16);
      }, Parallel.stepSpeed * 3));
  };
 ...
 */
Parallel.Level3.DisplayStudent = function(est, x, y, d, opt_angle) {
    ...
    if(Parallel.Level3.ReturnBook(x, y)){
        //Remove one book from the list and decrement the counter
        booksCounterNum--;
        //Move the student to the initial position and run the code again
        var svg = (document.getElementById('svgMaze'));
        Parallel.Level3.ResetOneStd(est);
        Parallel.Level3.RemoveBooks(svg, document);
        Parallel.Level3.Execute();
    }
};


Parallel.Level3.ResetOneStd = function(est){
    ...
    Parallel.Level3.DisplayStudent(est, student[est].startLoc.x, student[est].startLoc.y, Parallel.startDirection * 4);
 ...
 */
Parallel.Level3.ScheduleLook = function(d, x, y) {
 ...
 */
Parallel.Level3.ReturnBook = function(x, y){
 ...
 */
Parallel.Level3.RemoveBooks = function(svg, document){
    var book = 'book';
...
Parallel.Level3.NotDone = function(){
