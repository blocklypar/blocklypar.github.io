 ...
 */
Tasks.Level1.DrawMap = function(svg){

    //Create cenario:
    if (Tasks.Level1.VIEW.background) {
        var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
        tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Tasks.Level1.VIEW.background);
    ...
    if (Tasks.Level1.VIEW.graph) {
        ...
        for (var k = 0; k < Tasks.ROWS; k++) {
          ...
          h_line.setAttribute('stroke', Tasks.Level1.VIEW.graph);
        ...
        for (var k = 0; k < Tasks.COLS; k++) {
          ...
          v_line.setAttribute('stroke', Tasks.Level1.VIEW.graph);
    ...
    for (var y = 0; y < Tasks.ROWS; y++) {
        for (var x = 0; x < Tasks.COLS; x++) {
            ...
            // Tile sprite.
            var tile = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
            tile.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Tasks.Level1.VIEW.tiles);
    ...
    for (var y = 0; y < Tasks.ROWS; y++) {
        for (var x = 0; x < Tasks.COLS; x++) {
            if (Tasks.map[y][x] == Tasks.SquareType.START) {
                start_ = {x: x, y: y};
                student = new Tasks.Avatar(0, Tasks.startDirection, x, y, Tasks.Level1.VIEW.skin);
            } else if (Tasks.map[y][x] == Tasks.SquareType.FINISH) {
                finish_ = {x: x, y: y};
            } else if(Tasks.map[y][x] == Tasks.SquareType.ITEM) {
                if(Tasks.Level1.activity_.pos0 == 0)
                    Tasks.Level1.activity_.pos0 = {x: x, y:y};
                else
                    Tasks.Level1.activity_.pos1 = {x: x, y:y};
            }
        }
    }
};
 ...
 */
Tasks.Level1.AddSprites = function(svg, document){

    // Create finish marker
    var finishMarker = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
    finishMarker.id = 'finish';
    finishMarker.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Tasks.Level1.VIEW.finishMarker);
    ...
    pegmanIcon.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Tasks.Level1.VIEW.skin);
 ...
 */
Tasks.Level1.AddActivitySprites = function(){

    var svg = document.getElementById('svgMaze');

    var activity1 = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
    activity1.id = 'activity0';
    activity1.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Tasks.Level1.VIEW.activity);
    activity1.setAttribute('height', 45);
    activity1.setAttribute('width', 35);
    svg.appendChild(activity1);
    activity1.setAttribute('x', Tasks.SQUARE_SIZE * (Tasks.Level1.activity_.pos0.x + 0.5) -
    activity1.getAttribute('width') / 2);
    activity1.setAttribute('y', Tasks.SQUARE_SIZE * (Tasks.Level1.activity_.pos0.y + 0.6) - activity1.getAttribute('height'));


    var activity2 = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');
    activity2.id = 'activity1';
    activity2.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Tasks.Level1.VIEW.activity);
    activity2.setAttribute('height', 45);
    activity2.setAttribute('width', 35);
    svg.appendChild(activity2);
    activity2.setAttribute('x', Tasks.SQUARE_SIZE * (Tasks.Level1.activity_.pos1.x + 0.5) -
    activity2.getAttribute('width') / 2);
    activity2.setAttribute('y', Tasks.SQUARE_SIZE * (Tasks.Level1.activity_.pos1.y + 0.6) - activity2.getAttribute('height'));
};

Tasks.Level1.Reset = function(first){
    ...
    if (first) {
        ...
        pidList.push(setTimeout(function() {
            Tasks.Level1.Schedule([student.startLoc.x, student.startLoc.y, student.startDirection * 4],
                        [student.startLoc.x, student.startLoc.y, student.startDirection * 4 - 4]);
                        student.startDirection++;
        }, Tasks.stepSpeed * 5));

    } else {
        Tasks.Level1.DisplayStudent(student.startLoc.x, student.startLoc.y, Tasks.startDirection * 4);
    }
...
Tasks.Level1.Execute = function(){
    ...
    var interpreter = new Interpreter(code, Tasks.Level1.InitInterpreter);
    ...
    try {
        ...
        Tasks.result = Tasks.Level1.NotDone() ?
    ...
    Tasks.Level1.Reset(false);
    pidList.push(setTimeout(Tasks.Level1.Animate, 100));
};
 ...
 */
Tasks.Level1.InitInterpreter = function(interpreter, scope){

    var wrapper;

    wrapper = function(id) {
        Tasks.Level1.move(0, id);
    };
    interpreter.setProperty(scope, 'moveForward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Tasks.Level1.move(2, id);
    };
    interpreter.setProperty(scope, 'moveBackward',
        interpreter.createNativeFunction(wrapper));
        
    wrapper = function(id) {
        Tasks.Level1.turn(0, id);
    };
    interpreter.setProperty(scope, 'turnLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        Tasks.Level1.turn(1, id);
    };
    interpreter.setProperty(scope, 'turnRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
    return Tasks.Level1.isPath(0, id);
    };
    interpreter.setProperty(scope, 'isPathForward',
    interpreter.createNativeFunction(wrapper));
   
    wrapper = function(id) {
        return Tasks.Level1.isPath(1, id);
    };
    interpreter.setProperty(scope, 'isPathRight',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Tasks.Level1.isPath(2, id);
    };
    interpreter.setProperty(scope, 'isPathBackward',
        interpreter.createNativeFunction(wrapper));

    wrapper = function(id) {
        return Tasks.Level1.isPath(3, id);
    };
    interpreter.setProperty(scope, 'isPathLeft',
        interpreter.createNativeFunction(wrapper));

    wrapper = function() {
        return Tasks.Level1.NotDone();
    };
    interpreter.setProperty(scope, 'notDone',
        interpreter.createNativeFunction(wrapper));
    
    wrapper = function() {
        // return Tasks.Level1.GetActivity(student.startLoc.x, student.startLoc.y);
    };
...
Tasks.Level1.Animate = function(){
    ...
    switch (action[0]) {
        ...
        case 'look_north':
        Tasks.Level1.ScheduleLook(Tasks.DirectionType.NORTH, student.startLoc.x, student.startLoc.y);
        break;

        case 'look_east':
        Tasks.Level1.ScheduleLook(Tasks.DirectionType.EAST, student.startLoc.x, student.startLoc.y);
        break;
        
        case 'look_south':
        Tasks.Level1.ScheduleLook(Tasks.DirectionType.SOUTH, student.startLoc.x, student.startLoc.y);
        break;
        
        case 'look_west':
        Tasks.Level1.ScheduleLook(Tasks.DirectionType.WEST, student.startLoc.x, student.startLoc.y);
        break;
        
        case 'fail_forward':
        Tasks.Level1.ScheduleFail(true);
        break;
        
        case 'fail_backward':
        Tasks.Level1.ScheduleFail(false);
        break;
    ...
    pidList.push(setTimeout(Tasks.Level1.Animate, Tasks.stepSpeed * 5));
}
 ...
 */
Tasks.Level1.move = function(direction, id) {

    if (!Tasks.Level1.isPath(direction, null)) {
 ...
 * @param {string} id ID of block that triggered this action.
 */
Tasks.Level1.turn = function(direction, id) {
 ...
 */
Tasks.Level1.isPath = function(direction, id) {
 ...
 */
Tasks.Level1.Schedule = function(startPos, endPos) {

    var deltas = [(endPos[0] - startPos[0])/4, (endPos[1] - startPos[1])/4, (endPos[2] - startPos[2])/4];

    pidList.push(setTimeout(function() {
    Tasks.Level1.DisplayStudent(startPos[0] + deltas[0],
        startPos[1] + deltas[1],
        Tasks.constrainDirection16(startPos[2] + deltas[2]));
    }, Tasks.stepSpeed));

    pidList.push(setTimeout(function() {
        Tasks.Level1.DisplayStudent(startPos[0] + deltas[0] * 2,
            startPos[1] + deltas[1] * 2,
            Tasks.constrainDirection16(startPos[2] + deltas[2] * 2));
    }, Tasks.stepSpeed));

    pidList.push(setTimeout(function() {
    Tasks.Level1.DisplayStudent(startPos[0] + deltas[0] * 3,
        startPos[1] + deltas[1] * 3,
        Tasks.constrainDirection16(startPos[2] + deltas[2] * 3));
    }, Tasks.stepSpeed * 2));

    pidList.push(setTimeout(function() {
        Tasks.Level1.DisplayStudent(endPos[0], endPos[1],
 ...
 */
Tasks.Level1.ScheduleLook = function(d, x, y) {
 ...
 */
Tasks.Level1.ScheduleFail = function(forward) {
    ...
    Tasks.Level1.DisplayStudent(student.startLoc.x, student.startLoc.y + deltaY,
                        direction16);

    BlocklyGames.workspace.getAudioManager().play('fail', 0.5);
    pidList.push(setTimeout(function() {
        Tasks.Level1.DisplayStudent(student.startLoc.x, student.startLoc.y,
                            direction16);
    }, Tasks.stepSpeed));

    pidList.push(setTimeout(function() {
        Tasks.Level1.DisplayStudent(student.startLoc.x + deltaX,
    ...
    pidList.push(setTimeout(function() {
        Tasks.Level1.DisplayStudent(student.startLoc.x, student.startLoc.y, direction16);
    }, Tasks.stepSpeed * 3));
};
 ...
 */
Tasks.Level1.DisplayStudent = function(x, y, d, opt_angle) {
     ...
     */
    if(Tasks.Level1.isActivity(x, y)){
        Tasks.Level1.ActivityDone();
        Tasks.Level1.activity_.num++;
    }
 ...
 */
Tasks.Level1.ActivityDone = function(){

    var svg = document.getElementById('svgMaze');
    var activity = document.createElementNS(Blockly.utils.dom.SVG_NS, 'image');

    activity.id = 'activity';
    activity.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', Tasks.Level1.VIEW.activity);
    ...
    activity.setAttribute('x', Tasks.SQUARE_SIZE * (0.3 + 0.8) -
        activity.getAttribute('width') / 2);
          Tasks.Level1.activity_.pos = Tasks.Level1.activity_.pos + 1;
    activity.setAttribute('y', Tasks.SQUARE_SIZE * (Tasks.Level1.activity_.pos + 2.5) -
        activity.getAttribute('height'));
    
    //Remove activity from the maze
    var name = 'activity';
    var svg = document.getElementById('svgMaze');
    activity = document.getElementById(name.concat(Tasks.Level1.activity_.num.toString()));
    svg.removeChild(activity);

};

Tasks.Level1.NotDone = function(){
    return Tasks.Level1.activity_.num != 2;
};

Tasks.Level1.isActivity = function(x, y){

    if((x == Tasks.Level1.activity_.pos0.x) && (y == Tasks.Level1.activity_.pos0.y))
        return true;
    
    if((x == Tasks.Level1.activity_.pos1.x) && (y == Tasks.Level1.activity_.pos1.y))
        return true;
 ...
 */
Tasks.Level1.RemoveActivities = function(){

    var svg = document.getElementById('svgMaze');

    for(var i=0 ; i<Tasks.Level1.activity_.num; i++){
    ...
    Tasks.Level1.activity_.num = 0;
    Tasks.Level1.activity_.pos = 0;
};